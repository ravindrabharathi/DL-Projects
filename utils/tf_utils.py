...
@timer
def get_tf_dataset(recordsfile, batch_size, shuffle=False):
  ...
  dataset = dataset.map(map_func=parse_record,num_parallel_calls=CPU_CORES)
...
@timer
def get_tf_dataset_in_batches(recordstype='train', batch_size=128, shuffle=False):
  ...
  #create dataset 
  dataset = get_tf_dataset(recordsfile, batch_size,shuffle)
...
def get_train_ds(batch_size=128):
    train_ds = get_tf_dataset_in_batches('train', batch_size, True)
    return train_ds

#create test data
@timer
def get_eval_ds(batch_size=128):
    test_ds = get_tf_dataset_in_batches('test', batch_size, False)
    return test_ds
